...
\begin{abstract}
Positron Emission Tomography (PET) is a important molecular imaging tool widely used in medicine. Traditional PET systems  rely on complete detector rings for full angular coverage and reliable data collection. 
...
We first use an Attention U-Net model to recover sinograms from incomplete ones, and then use the OSEM algorithm for preliminary reconstruction, and finally applies UNet-diffusion architecture for fine restoration. Our method also uses neighboring axial slices and spectral transforms as auxiliary guidance to improve consistency on spatial and frequency domain. 
Our model is tested on public brain PET datasets, showing that the proposed method significantly outperforms existing approaches in metrics such as PSNR (30.92 dB) and SSIM (0.9708), successfully preserving key anatomical structures and tracer distribution features, thus providing an effective solution for incomplete-ring PET imaging.
\end{abstract}
...
This study solves these problems by introducing a new two-stage model for both sinogram and its reconstruction which can preserve details useful for accurate clinical diagnosis.
...
\textbf{Section \ref{chap:methods}} (\emph{Fundamentals of Diffusion Probabilistic Models}) details the background of Denoising Diffusion Probabilistic Models (DDPM) and how they apply to various image reconstruction tasks, establishing the theoretical foundation for our proposed method. \emph{Proposed two-stage Restoration Framework}) describes the detailed workflow of our method, including complete and incomplete ring geometric modeling, creation of list-mode and sinogram data, the two-stage design, auxiliary guidance modules, contrastive diffusion learning objectives, and implementation details.
...
% , and our method achieved an average PSNR of 35.6421dB in high angular loss regions (30°-60°), significantly outperforming traditional methods.
...
% Our method achieved an average SSIM of 0.9588 on the validation set, indicating that the reconstructed images successfully preserved key structural features of the original PET images.
...
% This demonstrates the effectiveness of our approach in restoring missing data and improving image quality.
...
\begin{table}[htbp]
    ...
    &PSNR (sinogram)& 38.4107 dB&0.2695 dB\\
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[ht]
    ...
    % The reconstructed image preserves key anatomical structures and tracer distribution features from the original image, illustrating the capacity to restore complete images from incomplete ring PET geometries. 
...
But our approach was still able to achieve PSNR values above 35 dB and SSIM scores above 0.95, indicating successful ability to recover missing infomation of key structural features.
